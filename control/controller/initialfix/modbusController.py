...
class ModbusController:
    ...
    def __init__(self, port: str, defaultSlaveID: int, baudrate: int = 9600, timeout: int = 1):
        self.port = port # This will safely hold '/dev/ttyRS485'
    ...
    def connect(self) -> bool:
        for attempt in range(5):
            # Resolve the port dynamically on EVERY connection attempt
            actual_port = self._get_actual_port()
            
            if self._port_exist(actual_port):    
                try:
                    # Use actual_port instead of self.port here
                    self.instrument = minimalmodbus.Instrument(actual_port, self.defaultSlaveID)
                    self.instrument.serial.baudrate = self.baudrate
                    self.instrument.serial.timeout = self.timeout
                    self.instrument.mode = minimalmodbus.MODE_RTU
                    self.isConnected = True
                    logger.info(f"Connected to {actual_port} (via {self.port}) on attempt {attempt+1}")
                    time.sleep(1)
                    return True
    ...
    def _ensureConnection(self):
        # Resolve the port dynamically before checking if it exists
        actual_port = self._get_actual_port()
        ...
        if self._port_exist(actual_port):
            ...
            logger.error(f"Port {actual_port} does not exist - waiting for detection...")
            return False
